...
/*	quit;*/
	...
	data fr1;
		set fr;
			if strip(product_line_group) = "&Product_line_group." and current_plc in ('E2', 'F0', 'F1', 'F2', 'F3', 'F4', 'F5', 'G0', 'G2') and strip(seasons)="&seasonality." then output;
	run;

	proc sql;
		create table regions4report as 
			select distinct 1 as order, 'Country' as type, territory, country from dmimport.country_lookup where region="&region." and region^=country /*exception for BI/BI/BI, will also work after split into regions and countries*/
			...
			union select distinct 3 as order, region as type, region as territory, region as country from dmimport.country_lookup where region="&region.";
	quit;
	...
	create table fr4 as
		select 	a.*,
				...
				case when ^missing(b.s3) then a.s3/b.s3
	...
	proc sql;
		...
		from (select order, series, country, sum(ytd) as ytd, sum(s3) as s3 from fr3 group by order, series, country) a
		left join (select series, sum(ytd) as ytd, sum(s3) as s3 from fr3 where country="&region." group by series) b on a.series=b.series;
	quit;
	...
	proc sql;
		...
		from (select order, species, country, sum(ytd) as ytd, sum(s3) as s3 from fr3 group by order, species, country) a
		left join (select species, sum(ytd) as ytd, sum(s3) as s3 from fr3 where country="&region." group by species) b on a.species=b.species;
	quit;
	...
	proc sql;
		...
		from (select order, country, sum(ytd) as ytd, sum(s3) as s3 from fr3 group by order, country) a
		left join (select sum(ytd) as ytd, sum(s3) as s3 from fr3 where country="&region.") b on 1=1;
	quit;
