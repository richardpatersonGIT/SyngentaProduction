...
%macro read_sales_history_weekly(last_season_year=, 
	...
	%do i=1 %to &flcnt.;
		...
		data sales_history_raw(keep=sls_org sls_off shipto_cntry material variety SchedLine_Cnf_deldte cnf_qty order_type mat_div rsn_rej_cd filename);
			set shw.&dsname.;
			filename="&dsname.";
		run;

		data sales_history_tmp(drop=sls_org sls_off shipto_cntry order_type rsn_rej_cd _cnf_qty);
			set Sales_history_raw(rename=(cnf_qty=_cnf_qty));
			length unique_code $10. year 8. week 8.;
			unique_code=cats(sls_org, sls_off, shipto_cntry);
			year=&year.;
			week=&week.;
			cnf_qty=input(_cnf_qty, comma15.2);
			if ((mat_div in ('6B', '6C') and order_type in ('ZYPD', 'ZFD1', 'ZYPL')) or (mat_div='6A' and order_type='YQOR')) and missing(rsn_rej_cd) and ^missing(SchedLine_Cnf_deldte) then output;
		run;
