...
%macro s960_upload_preparation(forecast_report_file=,
	...
	proc sql;
		...
		left join dmimport.BI_process_stage_split d on upper(strip(b.product_line)||'s') = upper(d.product_line) and b.species_code=d.species and b.product_form=d.process_stage
	...
	proc sql;
		...
		select * from forecast_report1 a
	...
	proc sql;
		create table forecast_report2 as 
			...
			left join dmimport.BI_seasonality b on upper(strip(a.product_line)||'s') = upper(b.product_line) and a.species=b.species
			where ^missing(b.month)
			order by a.product_line, a.species, a.variety, a.material, a.product_form, b.month, a.process_stage_percentage, a.redistribution, total_process_stage_percentage;
	quit;
...
%macro s960_generate_upload(forecast_report_file=,
	...
	proc sql;
		...
		select a.*, b.Netproposal0*total_percentage as material_month_net0, b.Netproposal1*total_percentage as material_month_net1, b.Netproposal2*total_percentage as material_month_net2, d.material_desc
		...
		left join dmimport.material_class_table d on c.material_bulk=d.material;
	quit;
...
%macro upload_s960();
	...
	proc sql noprint;
		select count(*) into :report_cnt from dmimport.s960_md;
	quit;
