...
%macro REPORT_SALES_VS_DEMAND();
  ...
  data SvsD_material(drop=rc fps_material_name bi_material_name);
    set SvsD_variety;
    length sub_unit 8. DM_Process_stage $5. product_form $25. proc_stage $3. process_stage $5. material_division $2. 
    ...
    if _n_=1 then do;
      declare hash fps_assortment(dataset: 'dmimport.FPS_assortment(rename=(PF_for_sales_text=product_form Curr_Mat_PLC=material_plc_current
        ...
        rc=fps_assortment.DefineData ('sub_unit', 'DM_Process_stage', 'product_form', 'material_division', 
                                      'material_plc_current', 'material_plc_future', 'material_plc_future_date');
        rc=fps_assortment.DefineDone();
      declare hash bi_assortment(dataset: 'dmimport.Material_class_table(rename=(division=material_division))');
        rc=bi_assortment.DefineKey ('material');
        rc=bi_assortment.DefineData ('sub_unit', 'proc_stage', 'material_division');
        rc=bi_assortment.DefineDone();
      declare hash fps_material_n(dataset: 'dmimport.FPS_assortment(rename=(Material_basic_description_EN=fps_material_name))');
        rc=fps_material_n.DefineKey ('material');
        rc=fps_material_n.DefineData ('fps_material_name');
        rc=fps_material_n.DefineDone();
      declare hash bi_material_n(dataset: 'dmimport.Material_class_table(rename=(Material_Desc=bi_material_name))');
        rc=bi_material_n.DefineKey ('material');
        rc=bi_material_n.DefineData ('bi_material_name');
        rc=bi_material_n.DefineDone();
    end;

    rc=fps_material_n.find();
    rc=bi_material_n.find();
    ...
    If region = 'FPS' then do;
      ...
      material_name=coalescec(bi_material_name, fps_material_name); 
    end;
  run;
  ...
  data SvsD_variety_name_global(drop=rc global_region);
  	set SvsD_units;
	length variety_name_global $60.;
	if _n_=1 then do;
	declare hash h_vng(dataset: 'dmproc.pmd_assortment(rename=(region=global_region variety_name=variety_name_global))');
	  rc=h_vng.DefineKey ('variety', 'global_region');
