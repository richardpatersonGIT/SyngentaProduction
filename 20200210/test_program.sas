...
%macro read_extrapolation_metadata();
  ...
  data extrapolation_report_md;
    set extrapolation_report_md_raw;
...
%macro extrapolation_report();
  ...
  proc sql noprint;
      ...
      select catx('_', region, product_line_group, "&mat_div_name.", seasonality, put(hist_season, 4.), cats("wk",put(week, z2.))) into :ext_report_name trimmed from extrapolation_report_md where ^missing(region);
  quit;
