...
%macro s961_upload_preparation(forecast_report_file=,
  ...
  proc sql;
    ...
    select distinct variety from forecast_report where variety not in (select variety from dmproc.pmd_assortment where region='FPS');
  quit;
...
/*Add product information from FPS and PMD*/
  proc sql;
    ...
    select a.*, b.species, b.series, b.variety_name, b.current_plc as var_current_plc, c.material_basic_description_en as material_name from orders3 a
...
/*Calculate sales per series for var plc F2 and F3*/
  ...
  proc sql;
    ...
    select region, order_month_season, species, dm_process_stage, sum(actual_sales) as sum_species from orders4
    where var_current_plc in ('F2', 'F3')
      group by region, order_month_season, species, dm_process_stage;
  quit;
  ...
  proc sql;
    ...
    select region, order_month_season, species, dm_process_stage, order_year, order_month, sum(actual_sales) as sum_species_month 
    from orders4 
    where var_current_plc in ('F2', 'F3')
    group by region, order_month_season, species, dm_process_stage, order_year, order_month;
  quit;
  ...
  proc sql;
    ...
    select a.region, a.order_month_season, a.species, a.dm_process_stage, a.order_year, a.order_month, 
    ...
    left join orders_average_species b on a.region=b.region and a.order_month_season=b.order_month_season and a.species=b.species and a.dm_process_stage=b.dm_process_stage;
  quit;
  ...
  proc transpose data=species_per_month out=species_per_month1(drop=_name_) prefix=M_;
    by region species dm_process_stage;
    id yearmonth;
...
%macro s961_create_upload_mat_perc(forecast_report_file=,
  ...
  data species_averages_info;
    retain region species dm_process_stage sum;
  ...
  proc sql;
    ...
    from replace a
    left join series_averages_info b on a.series=b.series and a.dm_process_stage=b.dm_process_stage
    left join species_averages_info c on a.species=c.species and a.dm_process_stage=c.dm_process_stage;
  quit;

  data replace2;
    retain region  Species  Series  variety  Variety_name  variety_plc total_demand  material  material_name  DM_Process_stage  material_plc  replaced_by_suggestion  REPLACE series_average_sum species_average_sum sum;
    set replace1;
  run;
...
%macro s961_generate_upload(forecast_report_file=,
  ...
  proc sql;
    ...
    select a.variety, a.material, b.series, b.species, c.dm_process_stage from series_mat_average a
      left join dmproc.PMD_Assortment b on a.region=b.region and a.variety=b.variety
      left join dmimport.fps_assortment c on a.region=c.region and a.variety=c.variety and a.material=c.material;
  quit;
  ...
  proc sql;
    ...
    select a.variety, a.material, b.species, b.species, c.dm_process_stage from species_mat_average a
      left join dmproc.PMD_Assortment b on a.region=b.region and a.variety=b.variety
      left join dmimport.fps_assortment c on a.region=c.region and a.variety=c.variety and a.material=c.material;
  quit;
...
/*  quit;*/
  ...
  proc sql;
    ...
    left join species_per_month b on a.species=b.species and a.dm_process_stage=b.dm_process_stage;
  quit;

  data orders_per_month1;
    set orders_per_month_norep
        orders_per_month_rep
        orders_per_month_avg_series(drop=series dm_process_stage yearmonth rename=(sum_series_month=sum_month sum_series=sum_ps series_month_percentage=month_percentage))
        orders_per_month_avg_species(drop=species dm_process_stage yearmonth rename=(sum_species_month=sum_month sum_species=sum_ps species_month_percentage=month_percentage));
  run;
