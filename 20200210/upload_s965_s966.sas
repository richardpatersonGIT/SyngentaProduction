...
%macro s965_s966_upload_preparation(forecast_report_file=,
  ...
  proc sql;
    ...
    select distinct a.variety, b.variety_name, b.current_plc as variety_plc, a.total_demand, c.material,  c.curr_mat_plc as material_plc 
  ...
  proc sql;
    ...
    select distinct a.*, b.curr_mat_plc as material_plc, b.variety, c.current_plc as variety_plc, d.total_demand from materials_no_delivery_window a
...
/*Add product information from FPS and PMD*/
  proc sql;
    ...
    select a.*, b.species, b.series, b.variety_name, b.current_plc as var_current_plc, c.material_basic_description_en as material_name, c.PF_for_sales_text from orders3 a
...
%macro s965_s966_create_upload_mat_perc(forecast_report_file=,
  ...
  data replace1;
    retain region  Species  Series  variety  Variety_name  variety_plc  material  material_name  mat_div  PF_for_sales_text  material_plc  replaced_by_suggestion  REPLACE  series_average_sum species_average_sum total_demand  sum;
    set replace;
  run;
...
%macro s965_s966_generate_upload(forecast_report_file=,
  ...
  proc sql;
    ...
    select a.variety, a.mat_div, a.material, b.series, c.PF_for_sales_text from series_mat_average a
      left join dmproc.PMD_Assortment b on a.region=b.region and a.variety=b.variety
      left join dmimport.fps_assortment c on a.region=c.region and a.variety=c.variety and a.material=c.material;
  quit;
  ...
  proc sql;
    ...
    select a.variety, a.mat_div, a.material, b.species, c.PF_for_sales_text from species_mat_average a
      left join dmproc.PMD_Assortment b on a.region=b.region and a.variety=b.variety
      left join dmimport.fps_assortment c on a.region=c.region and a.variety=c.variety and a.material=c.material;
  quit;
